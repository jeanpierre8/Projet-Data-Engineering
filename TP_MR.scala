...
object TP_MR {
  ...
  def nthA[T](x: Int, l: List[T]): Option[T] = (x compare 0, l) match {
    case (_, List()) => None
    case (_, Nil) => None
    case (-1, _) => None
    case (0, head :: tail) => Some(head)
    case (1, head :: tail) => nthA(x - 1, tail)
  }
  ...
  def reverseA[T](list: List[T]): List[T] = list.foldLeft(List[T]()) {(acc, element) => element :: acc}
  ...
  def salarySum(employees: List[Employee]): Double = employees.foldLeft(0.0) {(acc, element) => acc + element.salary}
  ...
  def addressOf(users: List[User]): List[String] = users.foldLeft(List[String]()) {(acc, element) => acc :+ element.address}
  ...
  def average(values: Iterator[Double]): Option[Double] = values.length match {
    case 0 => None
    case l => Some(values.fold(0.0) {(acc, element) => acc + element} / l)
  }
  ...
  implicit def tupleMonoid[A: Monoid, B: Monoid]: Monoid[(A, B)] =
    new Monoid[(A, B)] {
      ...
      override def combine(left: (A, B), right: (A, B)): (A, B) =
        (Monoid[A].combine(left._1, right._1), Monoid[B].combine(left._2, right._2))
    }
  ...
  def skiRatingAverage: Double = {
    ...
    val partitionedRatingWithOne: MapView[String, Iterable[(Double, Int)]] =
      partitions.view.mapValues(rates => rates.map(rate => (rate, 1)))
    ...
    val partitionedSumRatingsAndCount: MapView[String, (Double, Int)] =
      partitionedRatingWithOne.mapValues(rates => iterableWithCombineAll(rates).combineAll)

    // TODO phase 3 (Reduce): combine for all partitions the sum of ratings and counts
    val (rating, count) : (Double, Int) =
      iterableWithCombineAll(partitionedSumRatingsAndCount.values).combineAll
